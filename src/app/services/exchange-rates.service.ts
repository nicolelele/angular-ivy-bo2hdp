...
export class ExchangeRatesService {
  ...
  getRates(currency: string, page = 1): Observable<MultiRates> {
    const url = `${this.BASE_URL}${currency}`;
      ...
      map(response => {
        return {
          ...
          rates: Object.entries(response.rates)
            ...
            .slice(
              this.ITEMS_PER_PAGE * (page - 1),
              this.ITEMS_PER_PAGE * page
            ),

          pagesCount: Math.round(
            Object.keys(response.rates).length / this.ITEMS_PER_PAGE
          )
        };
      })
    );
