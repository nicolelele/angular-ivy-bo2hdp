...
export class ExchangeRatesTableComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(){
    this.refresh();
    this.refreshInterval = setTimeout(this.refresh, this.REFRESH_INTERVAL);
  }

  refresh() {
    this.exchangeRatesService.getRates(this.currency, this.currentPage).subscribe(response => {
      this.data = response;
    });
  };
